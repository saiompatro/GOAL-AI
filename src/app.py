...
# so a new league only needs an entry there, not here too.
from fetch_club_results import LEAGUES as _CLUB_LEAGUES
for _key, _info in _CLUB_LEAGUES.items():
    ARTIFACTS.append({"key": f"{_key}_results", "path": f"data/club/{_key}_results.csv",
                      "label": f"{_info['name']} historical results",
                      "producer": "fetch_club_results.py", "max_age_h": None, "optional": True})
