...
function normalizePlayerAttributes(value: unknown): PlayerAttributes {
  ...
  return {
    ...
    internationalReputation: readNumber(
      value.international_reputation ?? value.internationalReputation,
    ),
  };
}
...
export const listPlayers = cache(async (): Promise<Player[]> => {
  ...
  const insightById = new Map(playerInsights.map((insight) => [insight.entityId, insight.summaryText]));
  const playerResult = await supabase
    .from("players")
    .select("id, team_id, name, position, overall, club_name, age, potential, preferred_foot, value_eur, wage_eur, attrs")
