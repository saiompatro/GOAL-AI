            ...
            <article className="panel panel--player" key={player.id}>
              <div className="player-card__header">
                <div>
                  ...
                  <p className="section-copy">{player.clubName ?? "Club data unavailable"}</p>
                </div>
              ...
              <div className="detail-list">
                ...
                <div className="detail-row">
                  <span>Dribbling</span>
                  <strong>{player.attrs.dribbling}</strong>
                </div>
                <div className="detail-row">
                  <span>Defending</span>
                ...
                <div className="detail-row">
                  <span>Physic</span>
                  <strong>{player.attrs.physic}</strong>
                </div>
                <div className="detail-row">
                  <span>Height</span>
                  <strong>{player.attrs.heightCm > 0 ? `${player.attrs.heightCm.toFixed(0)} cm` : "—"}</strong>
                </div>
                <div className="detail-row">
                  <span>Value</span>
                  <strong>{formatCurrency(player.valueEur)}</strong>
                </div>
                <div className="detail-row">
                  <span>Wage</span>
                  <strong>{formatCurrency(player.wageEur)}</strong>
                </div>
              </div>

              <p className="player-card__insight">{player.insightSummary ?? describePlayer(player)}</p>
            </article>
          ))}
        </div>
      )}
