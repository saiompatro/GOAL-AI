...
def build_fixture_row(feats: pd.DataFrame, home: str, away: str,
    ...
    default_feature_values = {
        ...
        "def_mean": 70.0,
        "potential_mean": 72.0,
        "potential_top11_mean": 73.0,
        "value_mean_eur": 6_000_000.0,
        "value_top11_eur": 8_000_000.0,
        "wage_mean_eur": 35_000.0,
        "age_mean": 27.0,
        "height_mean": 181.0,
        "international_reputation_mean": 1.5,
        "dribbling_mean": 68.0,
        "physic_mean": 70.0,
    }
    for col, default_value in default_feature_values.items():
        row[f"home_{col}"] = get(h, f"home_{col}", default_value)
        row[f"away_{col}"] = get(a, f"away_{col}", default_value)
...
def predict_fixture(home: str, away: str, neutral: bool = True, stage: str = "FIFA World Cup") -> dict:
    ...
    metrics_data = json.loads((art / "metrics.json").read_text())
    chosen = metrics_data["chosen"]
    if chosen == "stack":
        import torch
        ...
        base_probs = [xgb_m.predict_proba(x.reshape(1, -1)),
        ...
        if "nn" in ensemble_members and (art / "nn.pt").exists():
            ...
            with torch.no_grad():
                nn_p = F.softmax(model(torch.tensor(Xs, dtype=torch.float32)), dim=1).numpy()
            base_probs.append(nn_p)
    ...
    return {
        ...
        "model_version": metrics_data["version"],
        "chosen_model": chosen,
