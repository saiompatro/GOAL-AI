...
def train_all(df: pd.DataFrame, cfg: dict) -> dict:
    ...
    lr = Pipeline([("sc", StandardScaler()),
                   ("lr", LogisticRegression(max_iter=2000, C=1.0, random_state=rs))]).fit(
    ...
    if torch is not None and nn is not None:
        ...
        ensemble_members.append("nn")

    # 5. Stacked ensemble: LR meta on top of the strongest available base learners.
    stack_tr = np.hstack([results["val_probs"][k] for k in ensemble_members])
    meta = LogisticRegression(max_iter=2000, C=1.0, random_state=rs).fit(stack_tr, split.y_val)
    stack_te = np.hstack([results["test_probs"][k] for k in ensemble_members])
    ...
    results["ensemble_members"] = ensemble_members
    results["split"] = split
    return results
...
def save_artifacts(results: dict, out_dir: Path, version: str) -> None:
    ...
    # NN state
    if torch is not None and "nn" in results["models"]:
        torch.save(results["models"]["nn"], out_dir / "nn.pt")
    # Metrics
    (out_dir / "metrics.json").write_text(json.dumps({
        "version": version,
        "chosen": results["chosen"],
        "ensemble_members": results.get("ensemble_members", ["xgb", "lgbm"]),
