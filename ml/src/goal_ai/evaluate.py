...
def write_model_card(results: dict, out_path: Path, version: str) -> None:
    ...
    for name, m in results["metrics"].items():
        val_tag = "*" if name == "stack" else ""
        rows.append(
            f"| {name:<10} | {m['val']['log_loss']:.4f}{val_tag} | {m['val']['brier']:.4f}{val_tag} | "
            f"{m['val']['macro_f1']:.3f}{val_tag} | {m['test']['log_loss']:.4f} | "
...
## Explainability
SHAP TreeExplainer applied to the XGBoost model for per-prediction and global feature importance. See `shap_summary.png` and the `shap` JSON column in Firebase `predictions`.
