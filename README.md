...
Cup** and top club leagues (**Premier League**, **La Liga**, with more on the
way). The browser app keeps each competition's data, models, and projects
...
| **La Liga** | Leagues → La Liga | League match predictor (Elo, form, morale, head-to-head, scoreline grid, match markets) |
...
data/    results.csv (1872–2026 internationals), squads.csv, clubelo_latest.csv,
         ...
         club/     premier_league_results.csv, la_liga_results.csv, ..._state.json
                   (one set of results/training-table/state per league)
...
models/  fifa_model.joblib, metrics.json, premier_league_model.joblib, la_liga_model.joblib
...
(fewer lopsided squad gaps, deeper benches), so both sit close to the
...
no other code changes. La Liga (added after Premier League) proved this out:
the front-end's league dropdown and match-predictor UI are already generic
over `/api/leagues`, so no UI changes were needed to add a second league —
next candidates are Serie A, Bundesliga, Ligue 1, and continental/international
tournaments (UEFA Champions League, Copa América).
...
- Home-advantage and K-factor are fixed constants tuned by eye on the Premier
  League and shared across leagues (including La Liga), not fit per league —
  a per-league tune is the next accuracy lever once a third league lands.
