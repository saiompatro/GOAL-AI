...
Cup** and club leagues — **Premier League** and **La Liga** so far. The browser
app keeps each competition's data, models, and projects together using a clear
...
| **La Liga** | Leagues → La Liga | League match predictor (same Elo/form/morale/H2H engine as Premier League) |
...
data/    results.csv (1872–2026 internationals), squads.csv, clubelo_latest.csv,
         ...
         club/     {premier_league,la_liga}_results.csv, ..._state.json (per-league)
...
Club football is far more competitively balanced than international
football (fewer lopsided squad gaps, deeper benches), so both leagues sit
close to the accuracy ceiling reported in prediction-market literature —
...
no other code changes; La Liga (`es.1`) was added this way. Serie A, Bundesliga
and Ligue 1 follow the same path. Continental cup competitions (UEFA Champions
League, Copa América) don't: they mix home/neutral venues and a group +
knockout structure, so they need their own feature/predict logic rather than
just a new `LEAGUES` entry.
...
- Home-advantage and K-factor are fixed constants tuned by eye on Premier
  League data, not fit per league — La Liga reuses the same constants for
  now; expect them to need retuning as more leagues are added.
