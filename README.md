...
| **La Liga** | Leagues → La Liga | League match predictor (same Elo/form/H2H engine as Premier League) |
...
data/    results.csv (1872–2026 internationals), squads.csv, clubelo_latest.csv,
         ...
         club/     premier_league_results.csv, la_liga_results.csv, ..._state.json
                   (one set of files per league key, club-league model)
...
football — there's less signal to extract than a WC where a top-10 nation can
...
no other code changes. La Liga was added exactly this way; Serie A, Bundesliga
and Ligue 1 are the same one-line addition away.
