...
Cup**, the **Premier League**, and **La Liga**. The browser app keeps each
competition's data, models, and projects together using a clear
...
| **La Liga** | Leagues → La Liga | League match predictor (scoreline grid, match markets, team form/H2H) |
...
data/    results.csv (1872–2026 internationals), squads.csv, clubelo_latest.csv,
         ...
         club/     ..._results.csv, ..._training_table.csv, ..._state.json per
                   league key (premier_league, la_liga)
...
GitHub mirror of football-data.co.uk, one file per league (`LEAGUES` dict:
`eng.1` Premier League, `es.1` La Liga). `src/club_features.py` builds the
training table per league; `src/predict_league.py` serves predictions via
`GET /api/leagues`, `GET /api/league_team`, `POST /api/predict_league` — the
front-end's League dropdown is populated straight from `/api/leagues`, so a
new league needs no UI changes.
...
no other code changes. Serie A (`it.1`), Bundesliga (`de.1`) and Ligue 1
(`fr.1`) are the next candidates from the same data source; UEFA competitions
(Champions League, Europa League) and continental tournaments (Copa América,
Euros) don't fit this domestic walk-forward pipeline as-is — they're
single/double-leg knockout plus group stages rather than a round-robin
season — and would need their own feature/prediction path.
...
- Home-advantage and K-factor are fixed constants shared across leagues, not
  fit per league — both Premier League and La Liga currently use the same
  `HOME_ADV=70`, `K=20`, which likely isn't each league's true optimum.
