...
Cup** and club leagues (**Premier League**, **La Liga**, with more on the way).
...
| **La Liga** | Leagues → La Liga | League match predictor (shares the same engine; player-level projects are Premier League-only for now) |
...
data/    results.csv (1872–2026 internationals), squads.csv, clubelo_latest.csv,
         ...
         club/     <league>_results.csv, ..._training_table.csv, ..._state.json
                   per league (premier_league, la_liga, ...)
...
| Signal | Source |
|---|---|
| **Elo** | Per-league K-factor and home-advantage bonus (see table below), goal-margin multiplier, computed match-by-match over 30+ seasons |
...
ceiling reported in prediction-market literature for domestic leagues —
there's less signal to extract than a WC where a top-10 nation can play a
part-timer squad.
...
with a per-league `home_adv`/`k`) plus a re-run of `fetch_club_results.py` →
`club_features.py` → `train_league.py` — no other code changes. Home-advantage
and K-factor are now per-league (`home_adv`/`k` in `LEAGUES`, read by
`club_features.py` and carried into each league's `state.json` for
`predict_league.py`) rather than one constant shared across leagues:

| League | K | Home-advantage |
|---|---|---|
| Premier League | 20 | 70 |
| La Liga | 20 | 60 |
...
- Home-advantage and K-factor are now per-league (`LEAGUES` in
  `fetch_club_results.py`), but each league's values are still tuned by eye
  rather than grid-searched against held-out log-loss.
- Only two of Europe's "big five" domestic leagues are covered so far
  (Premier League, La Liga); Serie A, Bundesliga and Ligue 1 are one
  `LEAGUES` entry away, and continental/international cups (UEFA Champions
  League, Copa América) need a different pipeline since they aren't a
